...
import { holidays, initialSwitchState } from "./api/holidays";
...
export default function Home() {
  ...
  // Decide if we have a legit param
  let initialShow = showList[showParam] ? showParam : "theOffice"; // default to the office for now
  const initialEpisodeState = showListPaths[initialShow];
  const initialShowState = Object.keys(showList).reduce((acc, curr) => {
    acc[curr] = curr === initialShow ? true : false;
    return acc;
  }, {});
        ...
        <Grid item xs={12} md={8}>
          <div className="multiselect">
            <Multiselect
              options={dataSource}
              selectedValues={[
                dataSource.find(episode => episode.id === initialShow)
              ]}
